...
exports.createWorkshop = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: 'Workshop created successfully. It is now pending admin approval.', workshopId: workshopDoc.id });
    } catch (err) {
...
exports.getAllWorkshops = async (req, res) => {
    try {
        ...
        let workshops = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        // Filter out pending and rejected workshops dynamically
        workshops = workshops.filter(w => w.status === 'approved' || !w.status);
        ...
        console.error('Get workshops error:', err.stack);
...
/* GET MY ORGANIZED WORKSHOPS */
exports.getMyOrganizedWorkshops = async (req, res) => {
    try {
        ...
        console.error('Get my organized workshops error:', err.message);
        res.status(500).json({ message: 'Server error' });
    }
};
...
exports.getPendingWorkshops = async (req, res) => {
    try {
        ...
        console.error('Get pending workshops error:', err.message);
        res.status(500).json({ message: 'Server error' });
    }
};
...
exports.updateWorkshopStatus = async (req, res) => {
    ...
    try {
        ...
        res.json({ message: `Workshop has been ${status}` });
    } catch (err) {
