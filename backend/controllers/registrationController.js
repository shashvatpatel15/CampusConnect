...
exports.registerForWorkshop = async (req, res) => {
    ...
    try {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            // Create a deterministic registration ID to prevent race condition multi-registrations
            const regDocId = `${req.user.id}_${workshopId}`;
            const newRegRef = registrationsRef.doc(regDocId);
            const regDoc = await transaction.get(newRegRef);

            if (regDoc.exists) {
                throw { status: 400, message: 'You are already registered (detected via transaction)' };
            }
